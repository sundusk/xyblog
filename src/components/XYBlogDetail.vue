<template>
    <div class="blog-detail">
      <div class="header-image">
        <h1>{{ blogTitle }}</h1>
        <p class="blog-date">{{ blogDate }}</p> <!-- 显示创建时间 -->
      </div>
    ...
    methods: {
      fetchBlogContent(blogId) {
        ...
        if (context.keys().includes(blogKey)) {
          ...
          this.blogTitle = blogId; // 使用文件名作为标题
          this.blogContent = marked(markdownContent); // 渲染Markdown内容
          this.blogDate = new Date().toISOString().split('T')[0]; // 设置创建日期
        } else {
